...
checkpoint = callbacks.ModelCheckpoint(filepath + f'{datetime.now():%Y-%m-%d_%H.%M.%S}' + '.h5', monitor='val_acc',
                                       verbose=1, save_best_only=True, mode='max', period=1)

t = time.time()
hist = custom_resnet_model.fit(X_train, y_train, batch_size=batch_trainsize, epochs=nb_epoch, verbose=1,
                               validation_data=(X_test, y_test), callbacks=[tb, checkpoint])
